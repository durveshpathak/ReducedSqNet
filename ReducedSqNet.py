...
def squeeze_netv_1_1(inputs, classes):
    ...
    weights = {'conv1': tf.Variable(tf.contrib.layers.xavier_initializer()([3, 3, 3, 64])),
               'conv10': tf.Variable(tf.contrib.layers.xavier_initializer()([1, 1, 256, num_classes]))}

    biases = {'conv1': tf.Variable(tf.contrib.layers.xavier_initializer()([64])),
              'conv10': tf.Variable(tf.contrib.layers.xavier_initializer()([num_classes]))}

    output = tf.nn.conv2d(inputs, weights['conv1'], strides=[1,2,2,1], padding='SAME', name='conv1')
    ...
    #output = tf.nn.max_pool(output, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name='maxpool3')
    ...
    output = tf.nn.conv2d(output, weights['conv10'], strides=[1, 2, 2, 1], padding='SAME', name='conv10')
    output = tf.nn.bias_add(output, biases['conv10'])
    #output = tf.nn.elu(output)
    output = tf.nn.avg_pool(output, ksize=[1,7, 7, 1], strides=[1, 2, 2, 1], padding='SAME', name='avgpool10')
...
def optimize(num_iterations):
    ...
    for i in range(total_iterations,
       ...
       # x_batch, y_true_batch = data.train.next_batch(train_batch_size)
        ...
        if i% 10000 == 0:
            save_path = saver.save(session, "Model2/Squeeze_netmod.ckpt")
